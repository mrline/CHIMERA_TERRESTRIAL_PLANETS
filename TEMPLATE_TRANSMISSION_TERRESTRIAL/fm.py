...
def tran(T, P, mmw,Ps,Pc,   H2O,CH4,CO2,O3,N2O,CO,amp,power,M,Rstar,Rp,xsects):
    ...
    Pgrid, Tgrid, wno, gord, wts, xsecarr=xsects
...
def xsects(wnomin, wnomax):
    ...
    ### Read in CK arrays
    # H2O
    file='../ABSCOEFF_TERRESTRIAL/H2O_HITRAN_CK_R100_20gp_50_30000wno.pic'
    P,T,wno,kcoeff,g,wts = load(open(file,'rb'), encoding='latin1')
    xsecarrH2O=10**(kcoeff-4.)
    #
    file='../ABSCOEFF_TERRESTRIAL/CH4_HITRAN_CK_R100_20gp_50_30000wno.pic'
    P,T,wno,kcoeff,g,wts = load(open(file,'rb'), encoding='latin1')
    xsecarrCH4=10**(kcoeff-4.)
    #
    file='../ABSCOEFF_TERRESTRIAL/CO2_HITRAN_CK_R100_20gp_50_30000wno.pic'
    P,T,wno,kcoeff,g,wts = load(open(file,'rb'), encoding='latin1')
    xsecarrCO2=10**(kcoeff-4.)
    #
    file='../ABSCOEFF_TERRESTRIAL/O3_HITRAN_CK_R100_20gp_50_30000wno.pic'
    P,T,wno,kcoeff,g,wts = load(open(file,'rb'), encoding='latin1')
    xsecarrO3=10**(kcoeff-4.)
    #
    file='../ABSCOEFF_TERRESTRIAL/N2O_02_HITRAN_CK_R100_20gp_50_30000wno.pic'
    P,T,wno,kcoeff,g,wts = load(open(file,'rb'), encoding='latin1')
    xsecarrN2O=10**(kcoeff-4.)
    #
    file='../ABSCOEFF_TERRESTRIAL/CO_HITRAN_CK_R100_20gp_50_30000wno.pic'
    P,T,wno,kcoeff,g,wts = load(open(file,'rb'), encoding='latin1')
...
def fx(x,xsects):
    ...
    #computing transmission spectrum-----------
    spec = tran(T,P,mmw, Pref,Pc, H2O, CH4,CO2,O3,N2O,CO,1E-50,0., M, Rstar, Rp,xsects)
    wno = spec[0]
    F = spec[1]
    atm=np.array([T,P])
    return F,wno,atm
